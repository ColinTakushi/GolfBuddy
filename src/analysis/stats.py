"""Golf scorecard statistics analysis module."""
from src.database.db import SessionLocal
from src.database.models import User, Scorecard
...
def get_all_users_summary() -> dict:
    ...
    try:
        ...
        for user in users:
            stats = get_user_breakdown(user.username, db)
            if "summary" in stats:
                summary["users"][user.username] = stats["summary"]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python -m src.analysis.stats <username>")
        print("\nAvailable users:")
        db = SessionLocal()
        try:
            users = db.query(User).all()
